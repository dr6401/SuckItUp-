...
public class DustPickup : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((transform.position - target.position).sqrMagnitude < minGetSuckedUpDistance * minGetSuckedUpDistance) // use .sqrMagnitude to bypass calculating sqrRoot of target.position and transform.position (operation .magnitude would need to calculate that)
        {
